...
const LAST_VISIT_KEY = 'under80_events_last_visit';
...
export default function Events() {
  ...
  const newEventsCount = useMemo(() => {
    ...
    return events.filter(e => new Date(e.date) > new Date(lastVisit) || e.status === 'upcoming').length;
  }, [events]);
                ...
                <div style={{ borderTop: '1px solid #e0e6e0', background: '#fafbfa' }}>
                  ...
                  {txs.length > 0 ? (
                    txs.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map(tx => (
                      <div key={tx.id} style={{ display: 'grid', gridTemplateColumns: '70px 1fr 100px', gap: 6, padding: '8px 14px', borderBottom: '1px solid #f0f0f0', alignItems: 'center', fontSize: 13 }}>
                        <span style={{ fontSize: 11, color: '#888' }}>{fmtShort(tx.date)}</span>
                        <div><div style={{ fontWeight: 600, fontSize: 13 }}>{tx.description}</div><div style={{ fontSize: 11, color: '#aaa', marginTop: 1 }}>{tx.category}</div></div>
                        <span style={{ textAlign: 'right', fontWeight: 700, fontSize: 13, color: tx.type === 'income' ? '#2E7D32' : '#C62828' }}>{tx.type === 'income' ? '+' : '-'}{tx.amount.toFixed(2)}</span>
                      </div>
                    ))
                  ) : (
