...
import db, {
  type Transaction, type GolfEvent,
...
export default function Transactions() {
  ...
  const [form, setForm] = useState<{
    eventId: string; type: 'income' | 'expense'; category: string; amount: string; description: string; date: string;
  }>({
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await db.transactions.update(editing.id, { ...data, id: editing.id });
    } else {
      await db.transactions.add({ ...data, id: 0 });
    }
    setShowForm(false);
    load();
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <button className="btn btn-primary btn-sm" onClick={() => openNew('expense')}>+ 记录</button>
      </div>
      ...
      <div className="card" style={{ background: overallBalance >= 0 ? '#E8F5E9' : '#FFEBEE', padding: '10px 14px', marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 20, fontWeight: 800, color: overallBalance >= 0 ? '#2E7D32' : '#C62828' }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {grouped.map((group, gi) => (
            <div key={gi}>
              {/* Month header */}
              <div style={{
                background: '#f5f7f5', padding: '8px 14px', fontSize: 13, fontWeight: 700,
                color: '#1B5E20', borderBottom: '1px solid #e0e6e0',
              ...
              {group.items.map(({ tx, balance }, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '60px 1fr 90px 22px',
                      gap: '4px',
                      padding: '10px 12px',
      ...
      {/* FAB */}
      <button className="fab" onClick={() => openNew('expense')}>+</button>
          ...
          <div className="modal-content">
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  className={`btn btn-sm ${form.type === 'income' ? 'btn-primary' : 'btn-outline'}`}
                  onClick={() => setForm({ ...form, type: 'income', category: INCOME_CATEGORIES[0] })}
                  style={{ flex: 1 }}
                >📈 收入</button>
                <button
                  className={`btn btn-sm ${form.type === 'expense' ? 'btn-primary' : 'btn-outline'}`}
            ...
            <div className="form-group">
              ...
              <input className="input" type="number" inputMode="decimal" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
            </div>
            ...
            <div className="form-group">
              ...
              <input className="input" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="例如：GREEN FEE 或 会员年费" />
            </div>
            ...
            <div className="form-group">
              ...
              <select className="select" value={form.eventId} onChange={e => setForm({ ...form, eventId: e.target.value })}>
