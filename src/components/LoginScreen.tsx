...
export default function LoginScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || (!forgot && !password.trim())) return;
    setError('');
    setBusy(true);
    try {
      if (forgot) {
        const { error: resetError } = await supabase.auth.resetPasswordForEmail(email.trim().toLowerCase(), { redirectTo: window.location.origin + window.location.pathname });
        if (resetError) throw resetError;
        setMessage(t('reset_email_sent'));
      }
      else if (isRegister) { await signUp(email.trim(), password); }
      else { await signIn(email.trim(), password); }
    } catch (err: any) {
      const msg = err?.message || err?.msg || '';
      if (forgot) setError(t('reset_send_failed'));
      else if (msg.includes('Invalid login') || msg.includes('Invalid Login')) setError(t('invalid_login'));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', maxWidth: 480, margin: '0 auto', padding: '0 16px' }}>
      ...
      <div className="card" style={{ width: '100%', maxWidth: 360 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, color: '#1B5E20', textAlign: 'center' }}>
          {forgot ? t('forgot_password') : isRegister ? t('register') : t('login')}
        </h2>
        <form onSubmit={handleSubmit}>
          ...
          {message && <p role="status" style={{ color: '#2E7D32', fontSize: 13 }}>{message}</p>}
          {error && (
          ...
          <button className="btn btn-primary btn-block" type="submit" disabled={busy} style={{ marginTop: 8 }}>
            {busy ? t('please_wait') : forgot ? t('send_reset_email') : isRegister ? t('sign_up') : t('sign_in')}
          </button>
        </form>
        {!forgot && <button className="btn btn-outline btn-block" style={{ marginTop: 12 }} onClick={() => { setForgot(true); setError(''); setMessage(''); }}>{t('forgot_password')}</button>}
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <button onClick={() => { if (forgot) setForgot(false); else setIsRegister(!isRegister); setError(''); setMessage(''); }} style={{ background: 'none', border: 'none', color: '#2E7D32', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            {forgot || isRegister ? t('have_account') : t('no_account')}
          </button>
        </div>
      </div>
    </div>
