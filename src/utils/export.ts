...
export async function exportToExcel() {
  ...
  // Add income/expense breakdown below
  ...
  const evRows = events
    ...
    .map(ev => {
      ...
      return {
        ...
        收入: getTotalIncome(evTxs),
        支出: getTotalExpense(evTxs),
        结余: getBalance(evTxs),
        备注: ev.notes,
      };
    });

  const ws3 = XLSX.utils.json_to_sheet(evRows, {
    header: ['名称', '日期', '地点', '状态', '收入', '支出', '结余', '备注'],
  });
  ...
  const mbRows = members.map(m => ({
    ...
    手机: m.phone,
    邮箱: m.email,
    状态: m.status === 'active' ? '活跃' : '停用',
    加入日期: m.joinDate,
    备注: m.notes,
  }));
  const ws4 = XLSX.utils.json_to_sheet(mbRows, {
    header: ['姓名', '手机', '邮箱', '状态', '加入日期', '备注'],
  });
  ...
  const monthRows = Array.from(monthMap.entries())
    ...
    .map(([month, data]) => ({
      ...
      收入: data.income,
      支出: data.expense,
      结余: data.income - data.expense,
      交易笔数: data.count,
    }));

  const ws5 = XLSX.utils.json_to_sheet(monthRows, {
    header: ['月份', '收入', '支出', '结余', '交易笔数'],
  });
